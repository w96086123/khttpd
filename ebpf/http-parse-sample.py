...
# initialize BPF - load source code from http-parse-sample.c
bpf = BPF(src_file="http-parse-sample.c", debug=0)
...
while 1:
    ...
    print("IP hdr length '%s'" % ip_header_length)

    iph_bytes = packet_bytearray[ETH_HLEN:]
    iph = struct.unpack("!BBHHHBBH4s4s", iph_bytes[:20])
    ipsrc = socket.inet_ntoa(iph[8])
    ipdst = socket.inet_ntoa(iph[9])
    print("IP src '%s'" % ipsrc, "IP dst '%s'" % ipdst)
    ...
    tcph_bytes = packet_bytearray[ETH_HLEN + ip_header_length :]
    tcph = struct.unpack("!HHLLBBHHH", tcph_bytes[:20])
    tcpsrcp = tcph[0]
    tcpdstp = tcph[1]
    print("TCP src port '%s'" % tcpsrcp, "TCP dst port '%s'" % tcpdstp)
